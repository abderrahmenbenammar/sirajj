...
export default function DashboardPage() {
  ...
  const activeCourses = courses.filter((c) => studentData.activeCourses.includes(c.id));
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-800/60 p-5">
            ...
            <div className="text-2xl font-bold text-gray-900 dark:text-white">{stats.completedCourses}</div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-800/60 p-5">
            ...
            <div className="text-2xl font-bold text-gray-900 dark:text-white">{stats.totalHoursLearned}h</div>
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">{t("ساعات التعلم", "Hours Learned")}</div>
          </div>
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-800/60 p-5">
            ...
            <div className="text-2xl font-bold text-gray-900 dark:text-white">{stats.certificatesEarned}</div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
              ...
              {activeCourses.map((course) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{t(course.instructor, course.instructorEn)}</p>
                    <div className="mt-2 flex items-center gap-2">
                      <div className="flex-1 h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                        <div className="h-full bg-emerald-600 rounded-full" style={{ width: `${course.progress || 0}%` }} />
                      </div>
                      <span className="text-xs font-medium text-emerald-600 dark:text-emerald-400">{course.progress || 0}%</span>
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            {/* Recent Activity */}
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-800/60 p-5">
              <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-4">{t("النشاط الأخير", "Recent Activity")}</h3>
                ...
                {studentData.recentActivity.map((activity, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <div className="w-8 h-8 rounded-lg bg-gray-50 dark:bg-gray-800 flex items-center justify-center shrink-0 mt-0.5">
                      {activity.type === "lesson" ? <PlayCircle size={14} className="text-emerald-500" /> : <HelpCircle size={14} className="text-amber-500" />}
                    </div>
                    <div>
                      <p className="text-xs font-medium text-gray-900 dark:text-white">{t(activity.title, activity.titleEn)}</p>
                      <p className="text-xs text-gray-400">{t(activity.course, activity.courseEn)}</p>
                    </div>
                  </div>
                ))}
              </div>
