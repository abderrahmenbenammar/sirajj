...
// Reusable exam-taking flow (detail -> taking -> result) built on the existing
// exams/attempts APIs. Rendering it inside a lesson page ("embedded") is what
// makes a lesson exam part of the lesson instead of a detached /exams page.
export interface ExamRunnerProps {
  examId: string;
  initialDetail?: ExamDetail | null;
  embedded?: boolean;
  continueHref?: string;
  continueLabel?: { ar: string; en: string };
  onPassed?: () => void;
}

export default function ExamRunner({ examId, initialDetail, embedded = false, continueHref, continueLabel, onPassed }: ExamRunnerProps) {
  const { t, lang } = useLang();
  const [phase, setPhase] = useState<Phase>(initialDetail ? "detail" : "loading");
  const [detail, setDetail] = useState<ExamDetail | null>(initialDetail ?? null);
  ...
  const retry = () => {
    ...
    setConfirming(false);
    setPhase("detail");
    if (!initialDetail) loadDetail();
  };

  const pageWrap = embedded ? "space-y-6" : "py-12 sm:py-16";
  const container = embedded ? "" : "max-w-3xl mx-auto px-4 sm:px-6 lg:px-8";
  const card = embedded ? "" : "bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-800/60 p-6 sm:p-8";
  ...
  if (phase === "error" || !detail) {
    return (
      <div className={embedded ? "py-6 text-center" : "py-20 text-center"}>
        <p className="text-gray-700 dark:text-gray-300 mb-4">{error || t("الاختبار غير موجود", "Exam not found")}</p>
        {!embedded && (
    ...
    return (
      <div className={pageWrap}>
        <div className={container}>
          <div className={`${card} ${embedded ? "text-center" : "text-center"}`}>
            {result.passed ? (
              <CheckCircle2 size={embedded ? 44 : 56} className="text-emerald-500 mx-auto mb-4" />
            ) : (
              <XCircle size={embedded ? 44 : 56} className="text-amber-500 mx-auto mb-4" />
            )}
            <h1 className={`${embedded ? "text-xl" : "text-2xl sm:text-3xl"} font-bold text-gray-900 dark:text-white mb-2`}>
            ...
            <div className={`${embedded ? "text-4xl" : "text-5xl"} font-bold text-gray-900 dark:text-white mb-2`}>{result.scorePercentage}%</div>
            ...
            {!result.passed && (
              <p className="mb-6 text-sm font-medium text-amber-700 dark:text-amber-400">
                {result.attemptsLeft > 0
                  ? t("لم تجتز الاختبار. يجب اجتياز الاختبار للمتابعة، ويمكنك المحاولة مرة أخرى.", "You did not pass the exam. You must pass it to continue, and you can try again.")
                  : t("لم تجتز الاختبار، واستنفدت جميع محاولاتك في هذا الاختبار.", "You did not pass the exam and have used all attempts for it.")}
              </p>
            )}
            {result.passed && embedded && (
              <p className="mb-6 text-sm font-medium text-emerald-700 dark:text-emerald-400">
                {t("أنت مستعد للانتقال للدرس التالي", "You can continue to the next lesson")}
              </p>
            )}
            {review.length > 0 && (
              <div className="text-start space-y-2 mb-8">
                {review.map((item) => (
                  <div key={item.questionId} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 text-sm">
                    {item.isCorrect ? (
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
              {result.passed && continueHref ? (
                ...
                >
                  {continueLabel ? t(continueLabel.ar, continueLabel.en) : t("متابعة", "Continue")}
    ...
    <div className={pageWrap}>
      <div className={container}>
        ...
        <div className={card}>
          ...
          <h1 className={`${embedded ? "text-lg" : "text-2xl sm:text-3xl"} font-bold text-gray-900 dark:text-white mb-4`}>
            {t(detail.titleAr, detail.titleEn)}
          </h1>
          ...
          {error && <p className="mb-4 text-sm text-red-600 dark:text-red-400">{error}</p>}
          {detail.lastResult?.passed ? (
            <p className="text-sm font-medium text-emerald-700 dark:text-emerald-400">
              {t("لقد اجتزت هذا الاختبار.", "You have passed this exam.")}
            </p>
          ) : !detail.available ? (
